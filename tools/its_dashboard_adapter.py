...
The adapter watches a Camera ITS output directory and exposes a compact JSON
status endpoint. It intentionally keeps the data contract small so the demo UI
can run while the real ITS output format evolves.
"""
...
def summarize_results(results_dir: Path) -> dict[str, Any]:
    ...
    if not results_dir.exists():
        return {
            ...
            "tests": {key: 0 for key in metric_counts},
            "source": str(results_dir),
    ...
    for file_path in find_text_files(results_dir):
        ...
        file_passes = len(PASS_RE.findall(text))
        file_fails = len(FAIL_RE.findall(text))
        pass_count += file_passes
        fail_count += file_fails

        metric = classify_metric(file_path, text)
        if metric:
            metric_counts[metric]["pass"] += file_passes
            metric_counts[metric]["fail"] += file_fails
...
class ItsHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        payload = json.dumps(summarize_results(self.results_dir), ensure_ascii=False).encode("utf-8")
...
def main() -> None:
    ...
    print(f"Serving {args.results_dir} at http://{args.host}:{args.port}/its-status.json")
