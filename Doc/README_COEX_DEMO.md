...
서버 실행 옵션은 기본적으로 `tools/its_monitor_server.cfg`를 참조합니다. 기본값은 현재와 동일하게 `/tmp`, `8765`, `1.0`이며, `results_dir`에는 `/tmp` 또는 특정 `/tmp/CameraITS_*` 결과 폴더를 지정할 수 있습니다. 현재 구동 중인 ITS 패키지 경로는 `ITS_Tools`에 입력할 수 있습니다.
